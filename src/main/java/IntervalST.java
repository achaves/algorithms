...
public class IntervalST<Value>  {
    ...
    private Value get(Node x, Interval1D interval) {
        ...
        int cmp = interval.LEFT_ENDPOINT_ORDER.compare(interval, x.interval);
    ...
    private Node remove(Node h, Interval1D interval) {
        ...
        int cmp = interval.LEFT_ENDPOINT_ORDER.compare(interval, h.interval);
    ...
    public Interval1D search(Node x, Interval1D interval) {
        while (x != null) {
            ...
            else if (x.left.max < interval.left())  x = x.right;
            else                                 x = x.left;
        }
        return null;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < points.size(); i++) {
            Interval1D interval = new Interval1D(points.get(i).x(), points.get(i).y());
            StdOut.println(interval);
            st.root = st.rootInsert(st.root, interval, "" + i);
        }
